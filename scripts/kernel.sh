...
# Optional linux-nanopi-r2c-minimal image variant (self-hosted kernel repo).

# Public pacman repo served by GitHub Pages from nanopi-r2c-kernel-arch.
R2C_KERNEL_REPO_URL="${R2C_KERNEL_REPO_URL:-https://yimgruui.github.io/nanopi-r2c-kernel-arch/aarch64}"
...
trust_r2c_kernel_key() {
    ...
    if [ -z "$expected" ]; then
        ...
        echo "Get it from your kernel repo release asset:" >&2
        echo "  gpg --show-keys --with-colons nanopi-r2c-kernel-arch.pub | awk -F: '/^fpr:/ {print \$10; exit}'" >&2
        exit 1
    fi
...
write_minimal_kernel_source_info() {
    ...
    repo="https://github.com/yimgruui/nanopi-r2c-kernel-arch"
